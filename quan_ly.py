...
# ==========================================
# 2. APP SOI CALO ĐỒ ĂN TRỰC TIẾP (KHÔNG DÙNG FIREBASE)
# ==========================================
...
# Ô nhập khóa AI an toàn (Tránh lỗi Github)
...
if anh_do_an:
    # Không lưu file! Đọc trực tiếp từ bộ nhớ
    ...
    if st.button("🔍 AI XUẤT CHIÊU PHÂN TÍCH"):
        if not api_key:
            ...
            with st.spinner("🤖 AI đang soi từng hạt cơm... Boss đợi 3 giây nhé..."):
                try:
                    # 1. Khởi động AI
                    genai.configure(api_key=api_key)
                    # Dùng model flash vì nó hỗ trợ đọc ảnh cực nhanh và thông minh
                    ...
                    # 3. Gửi cả Câu lệnh + Bức ảnh lên AI cùng 1 lúc
                    response = model.generate_content([lenh_cho_ai, img])
                    
                    # 4. In kết quả ra màn hình cực đẹp
                    st.balloons()
