...
genai.configure("AQ.Ab8RN6JVw2ktSy2bLkox82Jx_Dm0Wl_16vZWz7dEGoAQ5S0ZHA")
...
st.set_page_config(page_title="JBLMONEY Ultimate", page_icon="💎", layout="wide")

# =======================================
# 3. CSS LÀM ĐẸP GIAO DIỆN (SANG TRỌNG & ỔN ĐỊNH)
# =======================================
st.markdown("""
<style>
    /* Làm đẹp các khối hiển thị tiền tài dạng Hộp nổi */
    div[data-testid="metric-container"] {
        background-color: #1E1E2E; border-radius: 15px; padding: 20px;
        box-shadow: 0 4px 6px rgba(0,0,0,0.3); border: 1px solid #333;
    }
    /* Đổi màu Nút bấm thành gradient chuyên nghiệp */
    div.stButton > button {
...
# =======================================
# 5. SIDEBAR ĐIỀU KHIỂN & TRỢ LÝ AI GỐC
# =======================================
...
# Các Tab tính năng quay trở lại phía trên màn hình nằm ngang rộng rãi
...
with tab1:
    ...
    with st.form("form_nhap_du_lieu", clear_on_submit=True):
        ...
        with col1:
            ...
            danh_muc = st.text_input("Danh mục (Ăn uống, Di chuyển, Lương...)")
        with col2:
...
with tab2:
    ...
    if tong_chi > han_muc:
        ...
        <div style="text-align: center; background-color: #ff4b4b; padding: 20px; border-radius: 10px; color: white;">
            <h1 style="margin:0;">💣 BOM SẼ NỔ SAU: <span id="timer">15</span> GIÂY</h1>
...
with tab4:
    ...
    tong_bill = col_b1.number_input("Tổng tiền trên hóa đơn (VNĐ):", min_value=0, step=20000)
    ...
    tien_muc_tieu = st.number_input("Số tiền cần đạt được (VNĐ):", min_value=1, value=30000000, step=1000000)
    ...
    st.write(f"Đã gom: **{tien_tiet_kiem:,.0f} đ** / **{tien_muc_tieu:,.0f} đ** (Đạt {phan_tram:.1f}%)")

# --- TAB 5: SỔ CÁI EXCEL ---
with tab5:
    st.subheader("📋 Bảng quản lý dữ liệu gốc")
    st.info("Nhấp đúp vào ô để sửa nhanh số/chữ. Chọn ô vuông đầu dòng bấm Delete để xóa dòng.")
